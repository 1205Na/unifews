...
# ===================== 修复：get_numel() 返回元组 =====================
numel_w_adj, numel_w_weight = model.get_numel()
...
if logger.lvl_config > 1:
    ...
    # ===================== 修复：格式化不报错 =====================
    print(f"[Test]  time: {time_test:0.4f} s, MACs: {macs_test:0.4f} G, Num adj: {numel_a:0.3f} k, Num weight: {numel_w_weight:0.3f} k")
...
if logger.lvl_config > 2:
    ...
    hstr, cstr = logger_tab.str_csvg(data=args.data, algo=args.algo, seed=args.seed, thr_a=args.thr_a, thr_w=args.thr_w,
                                    ...
                                    time_test=time_test, macs_test=macs_test, numel_a=numel_a, numel_w=numel_w_weight,
